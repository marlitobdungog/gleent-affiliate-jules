...
import type { PartnerPayoutsData } from "@/types/partnerPortal"
import {
  Table,
  TableBody,
  TableCell,
...
export function PartnerPayouts() {
  const [data, setData] = useState<PartnerPayoutsData | null>(null)
  ...
  if (!data) {
    return (
      <div className="p-6">
        <p className="text-muted-foreground">Unable to load payouts.</p>
      </div>
    )
  }
                ...
                {items.map((payout) => (
                  <TableRow key={payout.id}>
                    ...
                    <TableCell className="text-right text-muted-foreground">
                      {payout.paid_at ?? payout.requested_at}
                    </TableCell>
                  </TableRow>
                ))}
              </TableBody>
